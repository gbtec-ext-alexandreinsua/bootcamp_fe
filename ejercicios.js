...
"2 es un número par."
*/
function verSinumeroParOImpar(){
    //TODO: Validar los decimales
    const result = window.prompt("Dame un número entero");
    const número = Number.parseInt(result);
    
    //El método Number.isNaN() determina si el valor pasado es NaN. Versión más robusta de la función global isNaN().
    if(Number.isNaN(número)){
        //if(!Number.isInteger(result)){    
        console.error("El número no es válido");
        //Devolver la ejecución a nivel superior, interrumpe la ejecución del segundo if
        return;
    }
